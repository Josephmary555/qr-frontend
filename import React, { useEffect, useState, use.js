...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        // The auth token is set globally by setAuthToken, so no need for explicit headers.
        const response = await axios.get('/api/events'); // Use proxy and correct plural endpoint
  ...
  return (
    <div className="p-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <Link to="create-event" className="bg-blue-500 text-white p-4 rounded hover:bg-blue-600 text-center">
          Create Event
        </Link>
        <Link to="import-users" className="bg-blue-500 text-white p-4 rounded hover:bg-blue-600 text-center">
          Import Users
        </Link>
        <Link to="generate-qr" className="bg-blue-500 text-white p-4 rounded hover:bg-blue-600 text-center">
          Generate QR Code
        </Link>
        <Link to="scan-qr" className="bg-blue-500 text-white p-4 rounded hover:bg-blue-600 text-center">
          Scan QR Code
        </Link>
        <Link to="design-card" className="bg-blue-500 text-white p-4 rounded hover:bg-blue-600 text-center">
          Design Card
        </Link>
        <Link to="reports" className="bg-blue-500 text-white p-4 rounded hover:bg-blue-600 text-center">
          Generate Report
        </Link>
        {role === 'super_admin' && (
          <Link to="register-admin" className="bg-blue-500 text-white p-4 rounded hover:bg-blue-600 text-center">
            Register Admin
          </Link>
        )}
        <Link to="notifications" className="bg-blue-500 text-white p-4 rounded hover:bg-blue-600 text-center">
          Notification Status
        </Link>
      </div>
